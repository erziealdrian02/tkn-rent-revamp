...
@section('content')
    <div class="page-header">
        ...
        <div class="page-header-actions">
            ...
            <a href="{{ route('rentals.create') }}" class="btn btn-primary btn-sm"><i class="bi bi-plus-lg me-1"></i>New
                Rental</a>
        </div>
    </div>
                    ...
                    <tbody id="rentalTableBody">
                        @forelse($rentals as $rental)
                            <tr>
                                ...
                                <td><a href="{{ route('projects.show', $rental->project_id) }}"
                                        class="cell-link">{{ $rental->project->name ?? '-' }}</a></td>
                                <td>{{ \Carbon\Carbon::parse($rental->start_date)->format('d M Y') }}</td>
                                <td>{{ \Carbon\Carbon::parse($rental->return_date)->format('d M Y') }}</td>
                                <td><span class="badge bg-light text-dark border">{{ $rental->items->count() ?? 0 }}</span>
                                </td>
                                <td>
                                ...
                                <td>
                                    <a href="{{ route('rentals.show', $rental->id) }}" class="btn-action" title="View"><i
                                            class="bi bi-eye"></i></a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="10" class="text-center text-muted py-4">No rentals found</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>

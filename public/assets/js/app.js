...
function checkAuth() {
  ...
  if (!user) {
    // Laravel middleware handles auth redirect; don't redirect to .html
    return null;
  }
  return user;
}

// logout() is overridden in app.blade.php to POST to Laravel /logout
...
function renderSidebar(user, activePage) {
  ...
  if (isDriver) {
    // Driver Portal
    html += sidebarLink('/driver-dashboard', 'bi-speedometer2', 'Dashboard', activePage);
    html += sidebarLink('/driver-deliveries', 'bi-truck', 'My Deliveries', activePage);
    html += sidebarLink('#', 'bi-person', 'Profile', activePage);
  } else {
    // Full sidebar
    html += sidebarLink('/dashboard', 'bi-speedometer2', 'Dashboard', activePage);

    html += sidebarSection('RENTAL');
    html += sidebarLink('/rentals', 'bi-file-earmark-text', 'Rentals', activePage);
    html += sidebarLink('/projects', 'bi-folder', 'Projects', activePage);
    html += sidebarLink('/deliveries', 'bi-truck', 'Deliveries', activePage);
    html += sidebarLink('/returns', 'bi-box-arrow-in-left', 'Returns', activePage);
    html += sidebarLink('/claims', 'bi-exclamation-triangle', 'Claims', activePage);

    html += sidebarSection('INVENTORY');
    html += sidebarLink('/equipment', 'bi-tools', 'Equipment', activePage);
    html += sidebarLink('/branches', 'bi-building', 'Branches', activePage);
    html += sidebarLink('/stock', 'bi-boxes', 'Stock', activePage);
    html += sidebarLink('/stock-transfer', 'bi-arrow-left-right', 'Stock Transfer', activePage);
    html += sidebarLink('/movements', 'bi-arrow-left-right', 'Movements', activePage);
    html += sidebarLink('/purchases', 'bi-cart', 'Purchases', activePage);
    html += sidebarLink('/goods-receipts', 'bi-box-seam', 'Goods Receipts', activePage);
    html += sidebarLink('/repairs', 'bi-tools', 'Repairs', activePage);
    html += sidebarLink('/maintenance', 'bi-wrench-adjustable', 'Maintenance', activePage);

    html += sidebarSection('MASTER DATA');
    html += sidebarLink('/customers', 'bi-people', 'Customers', activePage);
    html += sidebarLink('/drivers', 'bi-person-badge', 'Drivers', activePage);
    html += sidebarLink('/vehicles', 'bi-truck-front', 'Vehicles', activePage);
    html += sidebarLink('/accounts', 'bi-bank', 'Company Accounts', activePage);

    html += sidebarSection('FINANCE & BILLING');
    html += sidebarLink('/invoices', 'bi-receipt', 'Invoices', activePage);
    html += sidebarLink('/finance-ledger', 'bi-wallet2', 'Bank Ledger', activePage);

    html += sidebarSection('ADMINISTRATION');
    html += sidebarLink('/users', 'bi-person-gear', 'Users', activePage);
    html += sidebarLink('/roles', 'bi-shield-lock', 'Roles', activePage);
  }
...
function sidebarLink(href, icon, label, activePage) {
  ...
  const isActive = currentPath === path || currentPath.startsWith(path + '/') || activePage === label.toLowerCase();
...
function renderHeader(breadcrumbs, title) {
  ...
  let bcHtml = '<a href="/dashboard"><i class="bi bi-house"></i></a>';
